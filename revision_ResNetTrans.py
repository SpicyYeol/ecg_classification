...
class ResNetWithTransformer(nn.Module):
    def __init__(self, num_classes=5, transformer_layers=6, transformer_heads=16, dropout=0):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=512, nhead=transformer_heads, dim_feedforward=2048,
    ...
    def forward(self, x):
        ...
        # Flatten into sequence for Transformer
        b, c, h, w = x.size()
        ...
        # Take mean of Transformer output and pass through FC layer
...
# 모델 초기화 및 GPU로 이동
model = ResNetWithTransformer(num_classes=5, transformer_layers=6, transformer_heads=16, dropout=0.2)

# 훈련 파라미터
best_val_loss = float('inf')
best_val_accuracy=0
save_path = os.path.join('revision_models', "2DResNetTrans6.pth")
...
# optimizer = Ranger21(model.parameters(), lr=0.0001, weight_decay = 1e-5,
#                   eps = 1e-8, use_warmup=True,
#                   use_cheb=False,
#                   use_madgrad = False,
#                   num_epochs = epochs,
#                   using_gc=True, num_batches_per_epoch = len(train_loader))
optimizer = optim.RAdam(model.parameters(), lr=0.00005, weight_decay=1e-6)
